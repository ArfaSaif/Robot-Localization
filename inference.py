...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        probE_given_Xt = busters.getObservationProbability(noisyDistance, trueDistance)
        return probE_given_Xt
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            p =  newPosDist.keys()
            Pr_X_t_1_given_e_t_1 = self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        legalBoardPositions = self.legalPositions
        numPositions = len(legalBoardPositions)
        tv =[]
        for p in range(numParticles):
            l = legalBoardPositions[p % numPositions]
            tv.append(l)
        self.particles = tv
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particlePos in particles:
            weight = self.getObservationProb(observation, pacmanPos, particlePos, jailPos)
            distribution[particlePos] = weight + distribution[particlePos]
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        differentPositions = itertools.product(legalPos, repeat=numGhosts)
        differentPositions = list(differentPositions)
        lenDifferentPos = len(differentPositions)
        random.shuffle(differentPositions)
        newParticles = []
        for particle in range(numParticles):
            newParticles.append(tuple(differentPositions[particle % lenDifferentPos]))
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particlePos in particles:
            ...
            for i in range(numGhost):
                obv = observation[i]
                jailPos = jailPosition[i]
                particlePos_ = particlePos[i]
                weight = self.getObservationProb(obv, pacmanPos, particlePos_, jailPos)
                p = p * weight
            distribution[particlePos] = p + distribution[particlePos]
